(() => {
  ...
  // Must match STORAGE.saved in main.js
  const SAVED_KEY = "flash.es-en.saved.v1";
  ...
  function loadSaved() {
    ...
    return raw.map(normalizeWord).filter((w) => w.es && w.en);
  }
  ...
  function showAt(index, { preserveFlip = false } = {}) {
    const saved = loadSaved(); // freshest list (another tab may have updated storage)
    if (!saved.length) {
      cursor = 0;
      current = null;
  ...
  function setupEvents() {
    ...
    window.addEventListener("keydown", (e) => {
      ...
      // macOS: Backspace is the key next to Return; many keyboards label it “delete” for removal.
